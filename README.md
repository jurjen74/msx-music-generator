...
- **Export scope:** tone notes, per-channel volume, and rests on 3 channels, plus **drums** on a 4th `D` channel (MSX-Music → YM2413 rhythm section; PSG → noise generator, overlaid on the bass). PSG notes get a **decay envelope** so they pluck rather than drone; FM relies on its instrument envelopes. No SFX or PSG hardware-envelope (R13) effects yet; FM uses one built-in instrument per channel. (See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) to extend, [docs/MML-REFERENCE.md](docs/MML-REFERENCE.md#drums--channel-d) for drum syntax.)
