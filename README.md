...
- **Export scope:** tone notes, per-channel volume, and rests on 3 channels, plus **drums on MSX-Music** (a 4th `D` channel driving the YM2413 rhythm section — kick/snare/hi-hat/tom/cymbal). PSG has no drums; neither chip does SFX or PSG hardware envelopes yet. FM uses one built-in instrument per channel. (See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) to extend, [docs/MML-REFERENCE.md](docs/MML-REFERENCE.md#drums--channel-d-msx-music-only) for drum syntax.)
