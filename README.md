...
- **Export scope:** tone notes, per-channel volume, and rests, mapped to 3 channels. No noise/SFX or hardware envelopes yet. FM uses one built-in instrument per channel. (See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) to extend.)
...
- **Longer pieces (≈32+ bars) can desync.** Length goes up to 64 bars (the output budget scales so it isn't truncated), but the longer the piece, the more the model tends to give the three channels slightly different total lengths. The exporter then loops on the longest channel, so shorter ones fall silent before the repeat. The result panel shows each channel's length and flags a mismatch — regenerate or pick fewer bars for a tight loop. For game BGM, a well-synced 16–32 bar loop that repeats is usually the better choice. Longer tracks also produce bigger data — use **lVGM** (and, for very long music, a larger ROM target or banking).
