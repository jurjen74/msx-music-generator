...
- **Export scope:** 3 tone channels (notes, per-channel volume, rests) plus **drums** on a 4th `D` channel (MSX-Music → YM2413 rhythm section; PSG → noise generator over the bass). Also: **vibrato** (`~`), per-note **decay** on PSG (pluck), and an optional **loop point** (`/`, intro + looping body). Not yet: SFX, PSG hardware envelopes (R13), or mid-track FM instrument changes; FM uses one instrument per channel. (See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) to extend, [docs/MML-REFERENCE.md](docs/MML-REFERENCE.md#drums--channel-d) for drum syntax.)
